...
import { from, map, Observable, of, switchMap, throwError, take } from 'rxjs';
...
export class RechargeService {
    ...
    confirmRecharge(rechargeId: string, cardId: string, amount: number): Observable<void> {
        const rechargeRef = doc(this.firestore, `recharges/${rechargeId}`);

        return from(updateDoc(rechargeRef, {
            recarga_status: RechargeStatus.CONFIRMADA,
            payment_status: PaymentStatus.FOI_PAGO,
            confirmed_at: new Date(),
        })).pipe(
            take(1),
            switchMap(() => this.cardService.getCardById(cardId).pipe(take(1))),
            switchMap((card) => {
                if (!card) {
                ...
                return this.cardService.updateCardBalance(cardId, newBalance).pipe(
                    take(1),
                    switchMap(() => {
                        ...
                        const cardLabel = card.nickname || card.card_code;

                        return this.transactionService.createRechargeTransaction(
                            uid,
                            cardId,
                            amount,
                            cardLabel
    ...
    getMyRecharges(): Observable<Recarga[]> {
        ...
        const rechargeRef = collection(this.firestore, 'recharges');
        const rechargeQuery = query(rechargeRef, where('user_id', '==', uid));
    ...
    getRechargesByCard(cardId: string): Observable<Recarga[]> {
        const rechargeRef = collection(this.firestore, 'recharges');
        const rechargeQuery = query(rechargeRef, where('card_id', '==', cardId));
